...
int setLingerOnSocket(int sock, Boolean onoff, unsigned long timeoutMSELs)
{
	struct linger l;
	l.l_onoff = (unsigned long)(onoff ? 1 : 0);
	l.l_linger = (unsigned long)(INFINITE==timeoutMSELs ? 0xffff : ((timeoutMSELs&0x7FFFFFFF) / 1000));
	if(0==l.l_linger && 0!=timeoutMSELs)
		l.l_linger = 1;
	return setsockopt(sock, SOL_SOCKET, SO_LINGER, (char *)&l, sizeof(l));
}
